...
const double MyCar::MAX_FUEL_PER_METER = 0.0008;  /* [liter/m] fuel consumption */
...
MyCar::MyCar(TrackDesc *track, tCarElt *car, tSituation *situation)
{
	...
	CFRICTION = GfParmGetNum(car->_carHandle, RRTCAR_SECT_PRIV, RRTCAR_ATT_FMAGIC, (char *)NULL, 1.0f);
...
void MyCar::updateCa()
{
	...
	for (int i = 0; i < 4; i++)
		h += GfParmGetNum(me->_carHandle, WheelSect[i], PRM_RIDEHEIGHT, (char *)NULL, 0.20f);
	h *= 1.5;
	h = h * h;
	h = h * h;
	h = 2.0 * exp(-3.0 * h);
