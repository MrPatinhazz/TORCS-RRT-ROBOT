...
Pathfinder::Pathfinder(TrackDesc *itrack, tCarElt *car, tSituation *s)
{
	...
	if (isPitAvailable())
	{
		...
		s1 = (int)GfParmGetNum(car->_carHandle, RRTCAR_SECT_PRIV, RRTCAR_ATT_PITENTRY, (char *)NULL, (float)s1);
		e3 = track->getPitExitEndId();
		e3 = (int)GfParmGetNum(car->_carHandle, RRTCAR_SECT_PRIV, RRTCAR_ATT_PITEXIT, (char *)NULL, (float)e3);
...
void Pathfinder::plotPath(char *filename)
{
	...
	int Pathfinder::initLeft(int id, double w)
	{
		...
		if (beta < 0.0)
			printf("error in initLeft: turn > 360� ??\n");
		...
		if (tlen < 0)
			printf("error in initLeft: tlen < 0 ??\n");
	...
	int Pathfinder::initRight(int id, double w)
	{
		...
		if (beta < 0.0)
			printf("error in initRight: turn > 360� ??\n");
		...
		if (tlen < 0)
			printf("error in initRight: tlen < 0 ??\n");
...
void Pathfinder::smooth(int id, double delta, double w)
{
	...
	xp = x[2] = xo + delta * tr->x;
	yp = y[2] = yo + delta * tr->y;
	for (i = 0; i < 3; i++)
	{
		r = fabs(radius(x[i], y[i], x[i + 1], y[i + 1], x[i + 2], y[i + 2]));
		if (r < rp)
	...
	xm = x[2] = xo - delta * tr->x;
	ym = y[2] = yo - delta * tr->y;
	for (i = 0; i < 3; i++)
	{
		r = fabs(radius(x[i], y[i], x[i + 1], y[i + 1], x[i + 2], y[i + 2]));
		if (r < rm)
...
inline void Pathfinder::adjustRadius(int s, int p, int e, double c, double security)
{
	...
	if (deltacurvature > 0.000000001)
	{
		...
		else
		{
			...
			{
				if (oldlane < ExtLane)
					newlane = MAX(oldlane, newlane);
				else
					newlane = ExtLane;
			}
			if (1 - newlane < IntLane)
				newlane = 1 - IntLane;
		}
...
void Pathfinder::interpolate(int step)
{
	...
	{
		int i;
		for (i = step; i <= nPathSeg - step; i += step)
...
int Pathfinder::overtake(int trackSegId, tSituation *s, MyCar *myc, OtherCar *ocar)
{
	...
	else if (minTime < FLT_MAX)
	{
		...
		{
			if (track->getSegmentPtr((trackSegId + i) % nPathSeg)->getRadius() < myc->OVERTAKERADIUS)
				return 0;
		}
	}
	else
	...
	if (((o[collcarindex].mincorner < myc->CARWIDTH / 2.0 + myc->DIST) && (minTime < myc->TIMETOCATCH)) || !sidechangeallowed)
	{
		...
		if (!sidechangeallowed)
		{
			if (paralleldist > 1.5 * myc->CARLEN)
			{
				...
				{
					if (alpha < beta + myc->OVERTAKEANGLE)
						alpha = alpha + myc->OVERTAKEANGLE;
				}
				else
				{
					if (alpha > beta - myc->OVERTAKEANGLE)
			...
			else
			{
				...
				if (minorthdist > myc->OVERTAKEMINDIST)
		...
		if (car != myc->getCarPtr())
		{
			...
			if (track->isBetween(start, end, seg) && !(car->_state & (RM_CAR_STATE_DNF | RM_CAR_STATE_PULLUP | RM_CAR_STATE_PULLSIDE | RM_CAR_STATE_PULLDN)))
			{
				...
				if (k < 40)
				{
					...
					for (j = l; j < l + k; j++)
						o[n].dist += ps[j % nPathSeg].getLength();
					if (o[n].dist > k)
						o[n].dist = k;
				}
				else
				...
				for (j = 0; j < 4; j++)
				{
					...
					if (corner < o[n].mincorner)
						o[n].mincorner = corner;
					if (orthdist < o[n].minorthdist)
						o[n].minorthdist = orthdist;
				}
				n++;
			}
		}
